...
# Data transformation and standardization for human Pfam data
new_hum_pfam = np.log10(
    hum_pfam.loc[hum_pfam.sum(axis=1)[hum_pfam.sum(axis=1) > 0].index].T + 1e-9
).apply(zscore)
...
for x in tqdm(all_study):
    metrics_df = run_RF(x, new_hum_pfam)
    all_metrics_all.append(metrics_df)
    print(x, ":done")

# Concatenating all metrics
results_hm3_all = pd.concat(all_metrics_all)

# Saving the combined results
results_hm3_all.to_pickle(path_all + "combined_pred/pred_results_hm3_all.pkl")

# Aggregating and pivoting AUC results
results_hm3_auc = (
    results_hm3_all[['train', 'test', 'auc']]
    .groupby(['train', 'test'])
    .median()
    .reset_index()
...
results_hm3_prc = (
    results_hm3_all[['train', 'test', 'pr_auc']]
    .groupby(['train', 'test'])
    .median()
    .reset_index()
